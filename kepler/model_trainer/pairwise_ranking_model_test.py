...
"""Tests for pairwise ranking model."""
...
_MODEL_0 = pairwise_ranking_model.PairwiseRankingModel(
...
_MODEL_1 = pairwise_ranking_model.PairwiseRankingModel(
    test_util.TEST_METADATA_0, list(range(test_util.TEST_NUM_PLANS_1)),
    test_util.TEST_MODEL_CONFIG_1, test_util.TEST_PREPROCESSING_CONFIG_0)


class PairwiseRankingModelTest(parameterized.TestCase):

  @parameterized.named_parameters(
      ("model0", _MODEL_0),
      ("model1", _MODEL_1)
  )
  def test_basic_training(self, model):
    # Basic check that this model can train without raising errors.
    x = test_util.TEST_INPUT_PARAMS_0
    x = x + [np.zeros(4)] + x + [np.ones(4)]  # All comparing plan 0 to 1.
    y = np.zeros(4)
    model.get_model().fit(x, y, epochs=1)

  @parameterized.named_parameters(
      ("model0", _MODEL_0),
      ("model1", _MODEL_1)
  )
  def test_output_dimensions(self, model):
    self.assertEqual(int(model._ranker.output.shape[1]), 1)

  @parameterized.named_parameters(
      ("model0", _MODEL_0),
      ("model1", _MODEL_1)
  )
  def test_inference_dimensions(self, model):
  ...
  def test_prediction_dimensions(self, model):
    self.assertEqual(
        model.get_model_predictions(test_util.TEST_INPUT_PARAMS_0).shape,
        (4,))
  ...
  def test_activations(self, model, activation):
    for layer in model._ranker.layers:
  ...
  def test_num_layers(self, model, target_num_layers):
    self.assertLen([l for l in model._ranker.layers
