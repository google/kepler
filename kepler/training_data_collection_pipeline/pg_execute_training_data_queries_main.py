...
_EXECUTION_METHOD = flags.DEFINE_string(
    ...
    "Which execution method to use: regular to simply time the latency, explain, or explain_analyze."
)
_ITERATIONS = flags.DEFINE_integer(
    "iterations", 3,
    "The number of iterations to execute query (query plan, parameter binding) pairing."
)
_BATCH_SIZE = flags.DEFINE_integer(
    "batch_size", 10,
    "Batch of parameters for which to collect training data before checkpointing."
)
...
_QUERY_TIMEOUT_MULTIPLIER = flags.DEFINE_integer(
    ...
    "considered 'way too slow' during execution data collection for each set of"
    " parameter values. This input has an inverse multiplicative relationship "
    "with query_timeout_minimum_speedup_multiplier. The product will be clipped"
    "to [query_timeout_min_ms, query_timeout_max_ms].")

_QUERY_TIMEOUT_MIN_MS = flags.DEFINE_integer(
    "query_timeout_min_ms", 200,
    "The minimum timeout for each query execution to enable setting a low "
    "multiplier while balancing the risk of timeouts caused by system noise for "
    "very fast queries.")

_QUERY_TIMEOUT_MAX_MS = flags.DEFINE_integer(
    "query_timeout_max_ms", 60 * 1000,
    "The maximum timeout for each query execution to provide a hard-cap on the "
    "cost of very slow query plans.")

_QUERY_TIMEOUT_MINIMUM_SPEEDUP_MULTIPLIER = flags.DEFINE_integer(
    "query_timeout_minimum_speedup_multiplier", 1,
    "This factor describes the minimum speed up expected from a candidate plan "
    "to be considered an alternative to the default. Plans that do not provide "
    "this speed up are considered timed out. This input has an inverse "
    "multiplicative relationship with query_timeout_multiplier.")

_VERIFICATION_FILE = flags.DEFINE_string(
    "verification_file", None,
    "File containing verification results. If specified, we will only execute "
    "hints for which there are no failures.")

_NUM_INITIAL_DEFAULT_EXECUTIONS = flags.DEFINE_integer(
    "num_initial_default_executions", None,
    "How many parameters to initially execute default plans for to determine "
    "the tail latency parameters.")
...
_PLAN_COVER_NUM_PARAMS = flags.DEFINE_integer(
    ...
    "Use the first N parameters to compute the plan cover. If not specified,"
    " plan cover pruning won't be used.")
...
def _get_skip_indices(plan_hints: Dict[str, List[Dict[str, str]]]):
  ...
  if not _VERIFICATION_FILE.value:
    return query_id_to_skip_indices

  with open(_VERIFICATION_FILE.value) as f:
...
def main(unused_argv):
  ...
  query_id_to_skip_indices = _get_skip_indices(plan_hints)
  ...
  def execute_explain(query: str,
                      params: List[Any],
                      _: Optional[int] = None) -> Tuple[Any, None]:
    return query_manager.get_query_plan(query, params), None

  def execute_explain_analyze(query: str,
                              params: List[Any],
                              _: Optional[int] = None) -> Tuple[Any, None]:
